...
export const getDownloadURL = async (fileId: string) => {
    ...
    if (!iframeURLMatch) throw new Error('获取 iframeURL 失败')
    const iframeURL = baseURL + iframeURLMatch[1]
    return fetchIframe(iframeURL)
}

const fetchIframe = async (iframeURL: string) => {
    ...
    const getValue = (varName: string) => {
        ...
        if (!match) throw new Error(`获取 ${varName} 失败`)
        return match[1]
    }
    ...
    const getValueKey = (key: string) => {
        ...
        const match = html.match(regExp)
        if (!match) throw new Error(`获取 ${key} 的值对应的变量名失败`)
        return match[1]
    }
    const signMatch = html.match(/'sign':'(.*?)'/)
    if (!signMatch) throw new Error('获取 sign 失败')
    ...
    if (!postURLMatch) throw new Error('获取 postURL 失败')
    const postURL = postURLMatch[1]
    return ajaxm(postURL, params)
}

const ajaxm = async (url: string, params: string[][]) => {
    ...
    }).then(res => res.json()).catch(error => { throw new Error('POST 响应非 JSON') }) as { dom: string, inf: number, url: string, zt: number }
    if (data.dom && data.url) return `${data.dom}/file/${data.url}`
